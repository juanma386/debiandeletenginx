...
'
clear;
echo 'ESAhora pasamos a realizar el borrado del nginx
EN Now we go to perform the deletion of the nginx'
sudo apt-get remove nginx
echo 'ESremovido!, ahora vamos por las dependencias
EN removed !, now we go through the dependencies'
sudo apt-get remove --auto-remove nginx
echo 'ESahora vamos a limpiar el sistema purgando todo
EN now let's clean the system purging everything'
sudo apt-get purge nginx
echo 'ESla etapa de borrado extendida
